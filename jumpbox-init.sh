...
VERSION_JSON=$(wget -qO- ${GITHUB_OPTIONS} ${GIST}/raw/jumpbox-init-versions.json )
...
function github_asset {
    ...
    wget -qO- ${GITHUB_OPTIONS} ${ASSET_URL} | jq -r "${JQ_EXPR}"
}
set -e
...
VERSION=$(wget -qO- ${GITHUB_OPTIONS} https://api.github.com/repos/cloudfoundry/cli/releases/latest  | jq -r '.tag_name' | sed s/^v// )
...
VERSION=$(wget -qO- ${GITHUB_OPTIONS} https://api.github.com/repos/docker/docker-ce/releases/latest  | jq -r '.tag_name'   | sed s/^v//)
...
VERSION=$(wget -qO- ${GITHUB_OPTIONS} https://api.github.com/repos/sharkdp/bat/releases/latest  | jq -r '.tag_name'  )
